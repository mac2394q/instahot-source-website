...
class Grimlock_Query_Section_Widget_Fields extends Grimlock_Section_Widget_Fields {
	...
	public function __construct( $id_base = 'grimlock_query_section_widget' ) {
		...
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_meta_key_field'       ), 120, 2 );
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_meta_compare_field'   ), 130, 2 );
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_meta_value_field'     ), 140, 2 );
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_meta_value_num_field' ), 150, 2 );
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_posts_per_page_field' ), 160, 2 );
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_orderby_field'        ), 170, 2 );
		add_action( "{$this->id_base}_query_tab",      array( $this, 'add_order_field'          ), 180, 2 );
