...
class Grimlock_Section_Block extends Grimlock_Base_Block {
	...
	public function __construct( $type = 'section', $domain = 'grimlock' ) {
		...
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_thumbnail_field'           ), 100, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_separator'                 ), 110, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_title_field'               ), 120, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_subtitle_field'            ), 130, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_text_field'                ), 140, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_text_wpautoped_field'      ), 150, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_separator'                 ), 160, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_button_displayed_field'    ), 170, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_button_text_field'         ), 180, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_button_link_field'         ), 190, 1 );
		add_action( "{$this->id_base}_general_panel_fields", array( $this, 'add_button_target_blank_field' ), 200, 1 );

		// Layout tab
		add_action( "{$this->id_base}_layout_panel_fields",  array( $this, 'add_layout_field'              ), 100, 1 );
		add_action( "{$this->id_base}_layout_panel_fields",  array( $this, 'add_container_layout_field'    ), 110, 1 );

		// Style tab fields
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_background_image_field'    ), 100, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_separator'                 ), 110, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_margin_top_field'          ), 120, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_margin_bottom_field'       ), 130, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_padding_y_field'           ), 140, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_separator'                 ), 150, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_background_color_field'    ), 160, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_background_gradient_field' ), 170, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_separator'                 ), 180, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_border_top_width_field'    ), 190, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_border_top_color_field'    ), 200, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_border_bottom_width_field' ), 210, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_border_bottom_color_field' ), 220, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_separator'                 ), 230, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_title_format_field'        ), 240, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_title_color_field'         ), 250, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_subtitle_format_field'     ), 260, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_color_field'               ), 210, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_subtitle_color_field'      ), 270, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_separator'                 ), 280, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_button_format_field'       ), 290, 1 );
		add_action( "{$this->id_base}_style_panel_fields",   array( $this, 'add_button_size_field'         ), 300, 1 );
	}
	 ...
	 * @return array Modified array of block fields
	 */
	public function add_padding_y_field( $fields ) {
		$fields[] = $this->range_field( apply_filters( "{$this->id_base}_padding_y_field_args", array(
			'name'  => 'padding_y',
			'min'   => -25,
			'max'   => 25,
			'unit'  => '%',
	...
	public function render_block( $attributes, $content ) {
		...
		ob_start();
		do_action( 'grimlock_section', apply_filters( "{$this->id_base}_component_args", $this->get_component_args( $attributes ) ) );
		return ob_get_clean();
	}
	...
	public function sanitize_attributes( $new_attributes ) {
		...
		$attributes['text_wpautoped']      = filter_var( $new_attributes['text_wpautoped'], FILTER_VALIDATE_BOOLEAN );
		...
		$attributes['button_target_blank'] = filter_var( $new_attributes['button_target_blank'], FILTER_VALIDATE_BOOLEAN );
		$attributes['button_displayed']    = filter_var( $new_attributes['button_displayed'], FILTER_VALIDATE_BOOLEAN );
	...
	protected function get_thumbnail_url( $attributes ) {
		...
		if ( ! empty( $attributes['thumbnail'] ) ) {
			$thumbnail_url = wp_get_attachment_image_url( $attributes['thumbnail'], apply_filters( "{$this->id_base}_thumbnail_size", "thumbnail-{$attributes['layout']}", $attributes['layout'] ) );
		}
